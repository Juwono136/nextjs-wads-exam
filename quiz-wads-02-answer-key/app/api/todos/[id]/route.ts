...
import { handleApiError, notFoundResponse, unauthorizedResponse } from "@/lib/api-error";
...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    const existing = await prisma.todo.findFirst({ where: { id, userId: session.user.id } });
    if (!existing) {
      return notFoundResponse("Todo not found");
    }
